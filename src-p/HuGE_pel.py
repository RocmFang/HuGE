...
class Graph():
    ...
    def simulate_walks_parallel(self, nodes, comm_neighbor, r, min_L, h, num_workers):
        '''
        Multiprocessing
        '''
        G = self.G
        ...
        while delt_h > h:
            ...
            with Manager() as manager:
                ...
                for i in range(num_workers):
                    ...
                    p = Process(target=self.simulate_walks_process, args=(nodes[start:end], comm_neighbor, r, min_L,
                                                                          walks, walk_length_list))
                    p.daemon = True
                    p.start()
                    p_list.append(p)
                ...
                _walks = _walks + walks[:]
                _walk_len_list = _walk_len_list + walk_length_list[:]

                H.append(self.relative_entropy_text(_walks, p_deg_list))
                delt_h = abs(H[-1]-H[-2])

                inter = inter+1

                print inter, ':', round(delt_h, 6)

        return _walks, _walk_len_list


    def simulate_walks_process(self, nodes, comm_neighbor, r, min_L, walks, walk_length_list):
