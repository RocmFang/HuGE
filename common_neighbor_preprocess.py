...
def parse_args():
    ...
    parser.add_argument('--output1', nargs='?', default='../pre_data/CA-AstroPh_comneig1.txt',
                        help='output the common neighbors')
...
def common_neighbors_score(G, nodes):

    f = open(args.output1, 'w')

    for cur in nodes:
        for w in G[cur]:
            v_nei = G[cur].keys()
            v_nei.remove(w)
            comm_neighbor = len(set(v_nei).intersection(set(G[w].keys())))
            f.write(u"{} {} {}\n".format(str(cur), str(w), str(comm_neighbor)))
...
def main(args):
    ...
    common_neighbors_score(nx_G, nodes)
    time2 = time.time()
